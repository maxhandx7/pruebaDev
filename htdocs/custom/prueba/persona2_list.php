...
/**
 *   	\file       persona2_list.php
 *		\ingroup    prueba
 *		\brief      List page for persona2
 */
...
require_once __DIR__.'/class/persona2.class.php';
...
if ($enablepermissioncheck) {
	...
	$permissiontoadd = $user->hasRight('prueba', 'persona2', 'write');
	$permissiontodelete = $user->hasRight('prueba', 'persona2', 'delete');
} else {
	$permissiontoread = 1;
	$permissiontoadd = 1;
...
if (!isModEnabled("prueba")) {
	accessforbidden('Module prueba not enabled');
}
...
if (empty($reshook)) {
	...
	// Mass actions
	$objectclass = 'Persona2';
	$objectlabel = 'Persona2';
	$uploaddir = $conf->prueba->dir_output;
...
$title = $langs->trans("Persona2s");
//$help_url = "EN:Module_Persona2|FR:Module_Persona2_FR|ES:Módulo_Persona2";
$help_url = '';
$morejs = array();
$morecss = array();
...
if ($num == 1 && getDolGlobalInt('MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE') && $search_all && !$page) {
	...
	$id = $obj->rowid;
	header("Location: ".dol_buildpath('/prueba/persona2_card.php', 1).'?id='.((int) $id));
	exit;
}
...
$newcardbutton .= dolGetButtonTitle($langs->trans('New'), '', 'fa fa-plus-circle', dol_buildpath('/prueba/persona2_card.php', 1).'?action=create&backtopage='.urlencode($_SERVER['PHP_SELF']), '', $permissiontoadd);

print_barre_liste($title, $page, $_SERVER["PHP_SELF"], $param, $sortfield, $sortorder, $massactionbutton, $num, $nbtotalofrecords, 'object_'.$object->picto, 0, $newcardbutton, '', $limit, 0, 0, 1);

// Add code for pre mass action (confirmation or email presend form)
$topicmail = "SendPersona2Ref";
$modelmail = "persona2";
$objecttmp = new Persona2($db);
